...
describe('Audit', () => {
  ...
  it('pagina hacia atrás y adelante', async () => {
    ...
    expect(screen.getByText('1821110 de 90 · Página 1 de 3')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '← Anterior' })).toBeDisabled();
    expect(screen.getByRole('button', { name: 'Siguiente →' })).toBeEnabled();

    await user.click(screen.getByRole('button', { name: 'Siguiente →' }));
    await waitFor(() => expect(lastAuditUrl()).toBe('/api/audit?page=2&perPage=10'));
    expect(await screen.findByText('11821120 de 90 · Página 2 de 3')).toBeInTheDocument();
  ...
  it('deshabilita el avance en la última página', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Siguiente →' }));
    await screen.findByText('11821120 de 90 · Página 2 de 3');
    await user.click(screen.getByRole('button', { name: 'Siguiente →' }));
    await screen.findByText('21821130 de 90 · Página 3 de 3');
